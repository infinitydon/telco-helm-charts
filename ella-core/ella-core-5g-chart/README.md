...
StatefulSet (OrderedReady, 3 replicas)
  ...
  Raft (TCP 7000)  →  bound to N2 IP   (stable across pod restarts)
  API  (HTTP 5002) →  bound to Pod IP  (security: not 0.0.0.0)
  N2   (SCTP 38412) → macvlan on host interface
  N3   (GTP-U)      → macvlan on host interface
  N6   (UPF data)   → macvlan on host interface
```

**Key design decision**: Raft uses N2 IPs (from Whereabouts) instead of DNS or Pod IPs. N2 IPs are stable per pod name — they survive pod recreation and rescheduling. This is what makes HA failover work when PVCs are replaced.
...
Expected: 3 nodes, all `voter` suffrage, one `isLeader: true`, all `drainState: active`. Raft addresses should show N2 IPs (e.g. `10.10.0.10:7000`), not pod IPs.
...
# Check cluster members (raftAddress = N2 IPs from Whereabouts)
curl -s -H "Authorization: Bearer ${TOKEN}" \
...
**5. Verify stable N2 IPs — restarted pod keeps the same address:**

```bash
# raftAddress must still show N2 IPs (10.10.0.x), not pod IPs
curl -s -H "Authorization: Bearer ${TOKEN}" \
...
Expected: all three lines show stable N2 IPs like `10.10.0.10:7000`. These IPs survive pod restarts because Whereabouts assigns them deterministically per pod name. If any show pod IPs (e.g., `10.1.x.x`), the cluster is using ephemeral addresses and may break on restart.
...
# 3. Scale back up — node-3 gets a new PVC, fresh DB, joins with same N2 IP
